<?php
class At_RecentPosts_Widget extends WP_Widget {

	function At_RecentPosts_Widget() {
	
		$widget_ops = array(
		'description' => __('A widget for your footer that displays the recent posts','apptamin')
		);
		parent::WP_Widget(false, 'Apptamin - Footer Recent Posts',$widget_ops);
	}
...
function At_RecentPosts($args = array(), $interval = '') {
	...
	$request .= 'post_status="publish"
			AND comment_count > 0
		ORDER BY '.$wpdb->posts.'.post_date DESC LIMIT 0, ' . $postCount;
...
register_widget('At_RecentPosts_Widget');
